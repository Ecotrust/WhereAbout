...
$BODY$
  ...
  --Intersect each study area geometry with the shape geometry.  Take advantage of GiST index with && operation
  FOR rec IN SELECT a.the_geom FROM "gwst_shoreline" a WHERE shape_geometry && st_setsrid(st_box2d(the_geom),sa_srid) AND st_intersects( shape_geometry, the_geom) LOOP
