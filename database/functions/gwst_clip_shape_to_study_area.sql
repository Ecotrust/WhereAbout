-- Function: gwst_clip_shape_to_study_area(text, integer)

-- DROP FUNCTION gwst_clip_shape_to_study_area(text, integer);

CREATE OR REPLACE FUNCTION gwst_clip_shape_to_study_area(IN shape_geometry_text text, IN shape_srid integer, OUT overlaps_study_area boolean, OUT clipped_shape geometry)
  RETURNS record AS
$BODY$
  DECLARE
  ...
  BEGIN
	...
	number_of_sa := 0;
	overlaps_study_area := FALSE;

	--Intersect each study area geometry with the shape geometry.  
	FOR rec IN SELECT a.bounds FROM "gwst_region" a WHERE st_intersects(shape_geometry, bounds) LOOP
		sa_geom := rec.bounds;
		...
		IF (sa_geom IS DISTINCT FROM NULL AND (geometrytype(sa_geom) = 'POLYGON' OR geometrytype(sa_geom) = 'MULTIPOLYGON')) THEN
			overlaps_study_area := TRUE;
			intersected_geometry := (SELECT st_intersection(shape_geometry, sa_geom));
			...
			--Otherwise, union each intesection to create the new clipped geometry 

			IF number_of_sa = 0 THEN
				clipped_shape := intersected_geometry;

			ELSE
				IF (area(intersected_geometry) > area(clipped_shape)) THEN
					clipped_shape := intersected_geometry;
				END IF;
			END IF;
			--increment counter
	...
	IF (overlaps_study_area = TRUE) THEN
  ...
  COST 100;
ALTER FUNCTION gwst_clip_shape_to_study_area(text, integer) OWNER TO postgres;
